...
const processFile = (file, jobs, cb, reporter) => {
  ...
  jobs.forEach(async job => {
    ...
    // duotone
    if (args.duotone) {
      clonedPipeline = await duotone(args.duotone, job.file.extension, clonedPipeline);
    }
...
function queueImageResizing({ file, args = {}, reporter }) {
  ...
  // can I output in another directory?
...
async function notMemoizedtraceSVG({ file, args, fileArgs, reporter }) {
  ...
  return trace(tmpFilePath, optionsSVG).then(svg => optimize(svg)).then(svg => encodeOptimizedSVGDataUri(svg));
}

const memoizedTraceSVG = _.memoize(notMemoizedtraceSVG, ({ file, args }) => `${file.absolutePath}${JSON.stringify(args)}`);
